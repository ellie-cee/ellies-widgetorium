...
class Sorcistino {
    constructor(options) {
        ...
        this.screen = 0;
        this.data = {};
        document.querySelectorAll(".sorcistino-nav").forEach(nav=>{
            nav.addEventListener("click",event=>this.navigate(nav));

        });
        let perc = 100/(document.querySelectorAll("[data-sorcistino-screen]").length-1);
        document.querySelectorAll("[data-sorcistino-screen]").forEach((screen,index)=>{
            screen.setAttribute("data-sorcistino-screen",index);
            if (screen.getAttribute("data-last-screen")) {
              screen.setAttribute("data-progress","100");  
            } else {
              screen.setAttribute("data-progress",perc*index);
            }
            this.screens.push(new sorcistinoScreen(screen,index));
        });
        ;
        this.load_data();
        this.show(0);
    ...
    generate_navbar(root) {
        document.querySelectorAll(root).forEach(holder=>{
            ...
            `;
            holder.querySelectorAll(".sorcistino-menu-item").forEach(nav=>{
    ...
    navbar_template(index,title) {
        return `<li data-sorcistino-screen="${index}" class="sorcistino-menu-item disabled">
    ...
    generate_navbuttons(root) {
        document.querySelectorAll(root).forEach(holder=>{
            holder.innerHTML = `
                <div class="sorcistino-nav-holder" style="display: flex; justify-content: space-around;">
            ...
            `;
            holder.querySelectorAll(".sorcistino-nav-button").forEach(nav=>{
                nav.addEventListener("click",event=>this.navigate(nav));
            });
        });
    }
    ...
    show(screen) {
        ...
        if (this.screen>0) {
             document.querySelectorAll('.sorcistino-nav-button[data-screen-nav="back"]').forEach(button=>button.removeAttribute("disabled"));
        } else {
            document.querySelectorAll('.sorcistino-nav-button[data-screen-nav="back"]').forEach(button=>button.setAttribute("disabled",""))
        }
      
        //\/ Nav Buttons
        if (this.screen+1<this.screens.length && this.current_screen().completed()) {
            document.querySelectorAll('.sorcistino-nav-button[data-screen-nav="next"]').forEach(button=>button.removeAttribute("disabled"));
        } else {
            document.querySelectorAll('.sorcistino-nav-button[data-screen-nav="next"]').forEach(button=>button.setAttribute("disabled",""));
        }
        ...
        document.dispatchEvent(
            new CustomEvent("sorcistino.ScreenChanged",{bubbles:true,detail:this})
         );
      if (this.current_screen().is_final()) {
        document.dispatchEvent(
            new CustomEvent("sorcistino.FinalScreen",{bubbles:true,detail:this})
         );
      }
    }
    load_data() {
